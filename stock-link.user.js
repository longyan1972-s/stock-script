...
// @name         Stock Link Bridge (forked from WzSLinker)
...
// @changelog    [2026-08-31] 共5510只股票，版本升 8.7.64（修复与远端 main 的推送分叉，确保浏览器自动更新到最新清单）
...
// @connect      localhost
// @connect      127.0.0.1
// @updateURL    https://raw.githubusercontent.com/longyan1972-s/stock-script/main/stock-link.user.js
// @downloadURL  https://raw.githubusercontent.com/longyan1972-s/stock-script/main/stock-link.user.js
...
(function( ) {
    ...
    class ContextualWalker {
        ...
        areNodesAdjacent(node1, node2) {
            // 只要任一节点在表格单元格内，两者必须在同一个单元格
            const cell1 = node1.parentNode?.closest?.('td, th');
            const cell2 = node2.parentNode?.closest?.('td, th');
            if (cell1 || cell2) {
                if (cell1 !== cell2) return false;
            }
            // 沿 DOM 树从 node1 向 node2 行走，只允许穿过内联元素
            let walk = node1;
            while (walk) {
                const sib = walk.nextSibling;
                if (sib) {
                    if (sib === node2) return true;
                    if (sib.nodeType === Node.TEXT_NODE) return false;
                    // 元素节点：内联可穿过，块级不可
                    const cs = getComputedStyle(sib);
                    if (cs.display.includes('inline')) {
                        if (sib.contains(node2)) return true;
                        // 跳过 inline 包装元素（如 <a>, <span>），继续在该层级找
                        walk = sib;
                        continue;
                    }
                    return false; // 块级元素，不跨
                }
                // 没有 nextSibling，往上走一层（只允许内联元素的父节点）
                const parent = walk.parentNode;
                if (!parent || parent === this.root || BLOCK_TAGS.has(parent.tagName)) return false;
                walk = parent;
            }
            return false;
        }
    ...
    function applyHighlight(context, regex) {
        // 收集所有匹配项，按偏移量从右向左排序处理，
        // 防止左侧匹配的 DOM 替换导致右侧匹配偏移量失效
        const matches = [];
        let match;
        regex.lastIndex = 0;
        ...
        for (const { text: matchText, start: startIndex } of matches) {
    ...
    function sendToLocalBridge(stockCode) {
        ...
        GM_xmlhttpRequest({
            method: "POST",
            url: "http://127.0.0.1:5680/api/link",
